...
本文介绍了一个基于长短期记忆网络(LSTM)的量化交易预测模型，该模型旨在预测股票价格的涨跌方向并生成交易信号。模型通过丰富的特征工程和优化的网络架构，将股票预测从传统的价格回归问题转变为方向分类问题，显著提升了预测准确率和交易策略的有效性。实验结果表明，优化后的模型在测试集上达到了57.14%的方向预测准确率，产生的交易策略年化收益率为17.71%，夏普比率为0.13，明显优于初始模型49.06%的预测准确率水平。
